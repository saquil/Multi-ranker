...
# aggregate the personalized evaluations of the multi-ranker models across the dataset splits for selected validation/test, number of pairs, batch size, lambda and preference set combination options
...
for pref in npref:
	for l in lbda:
		...
		for split in range(args.nsplits):
			...
			combine = list(hist['combine'].keys())
			kendall_local.append(np.array([ e[0] for e in hist['combine'].values() ]))
			spearman_local.append(np.array([ e[1] for e in hist['combine'].values() ]))
		...
		if args.metric == 'kendall':
			for i in range(kendall_local.shape[1]):	
				print("The combination: " + str(combine[i]))	
				print("Kendall tau personalized validation-test: [%.8f/%.8f]" % (kendall_mean_local[i], kendall_std_local[i]))	

		elif args.metric == 'spearman':
			for i in range(spearman_local.shape[1]):
				print("The combination: " + str(combine[i]))
				print("Spearman rho personalized validation-test: [%.8f/%.8f]" % (spearman_mean_local[i], spearman_std_local[i]))
